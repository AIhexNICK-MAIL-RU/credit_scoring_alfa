"""Fast CPU sequence model: sum of per-feature embeddings per timestep (cheap) + Conv1d
over credit-history + masked mean/last pooling + MLP. Reliable & fast (~0.7s/batch).
Train on fold!=0, val on fold0. Saves OOF(fold0)+test preds. Streams val AUC."""
...
TOTAL=tot
Xtr=np.load("seq_train.npy")
Xtr=np.where(Xtr>0,Xtr+offsets,0).astype(np.int16)
...
class Net(nn.Module):
    def __init__(s):
        super().__init__()
        s.emb=nn.Embedding(TOTAL+1,D,padding_idx=0)
    ...
    def forward(s,x,m):                       # x [B,L,F]; m [B,L,1]
        e=s.emb(x).sum(2)                     # [B,L,D] sum of feature embeddings (pads=0)
        h=s.act(s.proj(e)).transpose(1,2)
        h=s.act(s.c1(h)); h=s.act(s.c2(h)).transpose(1,2)
...
def train_epoch(net,idx,opt,bs):
    ...
    for s in range(0,len(idx),bs):
        b=idx[order[s:s+bs]]
        xb=torch.from_numpy(Xtr[b].astype(np.int64)).to(DEV); mb=torch.from_numpy(Mtr[b]).unsqueeze(-1).to(DEV); yb=torch.from_numpy(y[b]).to(DEV)
...
def predict(net,Xs,Ms,bs=16384):
    ...
    with torch.no_grad():
        for s in range(0,len(Xs),bs):
            xb=torch.from_numpy(Xs[s:s+bs].astype(np.int64)).to(DEV); mb=torch.from_numpy(Ms[s:s+bs]).unsqueeze(-1).to(DEV)
            out[s:s+bs]=torch.sigmoid(net(xb,mb)).cpu().numpy()
    return out
...
for ep in range(EPOCHS):
    t0=time.time(); tl=train_epoch(net,tri,opt,BS)
