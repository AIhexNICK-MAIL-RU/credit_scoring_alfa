"""Predict test for NN2 from saved weights (nn3_state.pt). Memory-safe: int8 + per-batch shift."""
...
net=Net(); net.load_state_dict(torch.load("nn3_state.pt")); net.eval()
...
np.save("test_nn3.npy",out)
print("test_nn3 saved mean",round(out.mean(),4),"n",len(out))
