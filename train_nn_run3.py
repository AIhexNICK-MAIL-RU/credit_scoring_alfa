...
torch.set_num_threads(10); torch.manual_seed(456); np.random.seed(456)
T0=time.time()
...
tri=np.where(fold!=1)[0]; vai=np.where(fold==1)[0]
...
for ep in range(EPOCHS):
    ...
    if a>best:
        ...
        oof=np.full(len(y),np.nan); oof[vai]=best_val; np.save("oof_nn3.npy",oof); torch.save(best_state,"nn3_state.pt")
    else:
        bad+=1
        if bad>=3: log("early stop"); break
log(f"=== NN best val(fold0) AUC = {best:.5f} ===")
net.load_state_dict(best_state)
oof=np.full(len(y),np.nan); oof[vai]=best_val; np.save("oof_nn3.npy",oof)
del Xtr, Mtr
Xte=load_shift("seq_test.npy"); Mte=(Xte!=0).any(-1).astype(np.float32)
tpred=predict(net,Xte,Mte); np.save("test_nn3.npy",tpred)
